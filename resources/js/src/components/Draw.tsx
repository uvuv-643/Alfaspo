...
import {
    ...
    getLengthWindow, getPanelLocation, isPointsSame,
    isValidPolygon,
...
interface DrawProps {
    ...
    preferSize : number,
    updateCoverageData: (number1 : number, number2 : number, number3 : number, number4 : number, string1 : string) => void,
    heightLowering ?: number,
    setHeightLowering : (data : number) => void,
    setSquare : (data : number) => void,
    weight : number
...
function Draw(props : DrawProps) {
    ...
    const [finishedBuilding, setFinishedBuilding] = useState<boolean>(props.finishedBuilding)
    ...
    const [createdPolygon, setCreatedPolygon] = useState<boolean>(false)
    ...
    function handleClickOnArea() {
        if (cursorPosition && !inactiveLine && !finishedBuilding && !wasMovedMouse && !createdPolygon && !angleMode) {
    ...
    const handleAngleDone = () => {
        setAngleMode(false)
        setCreatedPolygon(false); setFinishedBuilding(true);
    }
    ...
    useEffect(() => {
        props.setFinishedBuilding(finishedBuilding)
    }, [finishedBuilding])
    ...
    useEffect(() => {
        if (!angleMode && !moveMode && finishedBuilding && actualAngle !== undefined) {
            if (props.width && props.height && props.margin && props.points.length) {
                let panelLocationData = getPanelLocation(props.points, Number(props.material?.id), props.width.value, props.height.value, props.margin.value, actualAngle, P)
                props.updateCoverageData(panelLocationData.totalPanelLength, panelLocationData.totalStringerLength, panelLocationData.totalStringerCountActual, panelLocationData.totalConnectorsCount, panelLocationData.svg)
                setCoverage(panelLocationData.svg)
            }
        }
    }, [angleMode, moveMode, finishedBuilding, actualAngle, P, props.points])

    useEffect(() => {
        if (props.angle !== null && actualAngle === undefined) {
    ...
    useEffect(() => {
        if (actualAngle !== undefined) {
            props.setSelectedAngle(actualAngle)
        }
    }, [actualAngle])
    ...
    if (finishedBuilding) {
        return <div className="PageWrapper">
            <div className="PageWrapper__Sidebar PageWrapper__Sidebar--Draw">
                <Sidebar
                    handleBack={props.handleBack}
                    handleNext={props.handleNext}
                    currentPage={props.currentPage}
                    selectedValues={props.selectedValues}
                    selectedDropping={props.heightLowering}
                    setSelectedDropping={props.setHeightLowering}
                    square={square}
                    weight={props.weight}
                />
            </div>
        ...
        <div className="Draw" onClick={handleClickOnArea} onWheel={scrollHandler} onMouseMove={mouseMoveHandler} onMouseEnter={(event : React.MouseEvent) => setRect(event.currentTarget.getBoundingClientRect())}>
            ...
            <div className="Draw__Hints">
                <div className="Draw__Hints__Wrapper">
                    ...
                    <button className="Draw__Hints--Hint" onClick={handleBackToSize}>Інструкція</button>
                </div>
                <div className="Draw__Hints__Wrapper">
            ...
            {(
                (props.points.length > 2 && isPointsSame(props.points[0], props.points[props.points.length - 1])) && (
                    <div className="Draw__Hints--Created">
                        <div className="Draw__Hints--Angle__Again" onClick={() => { setMoveMode(true); }}>Рухати кут</div>
                        <div className={"Draw__Hints--Angle__Next _active"} onClick={() => { setAngleMode(true); }}>
                            Задати Напрямок
                            <img src="/images/draw/next.svg" alt="#" />
                        </div>
                    </div>
